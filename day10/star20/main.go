...
func main() {
	...
	fmt.Println("Inner symbols:", path.computeIncluded())
}

func (p *Path) computeIncluded() int {
	trailing := 0
	for i := 0; i < len(p.x); i++ {
		if i == len(p.x)-1 {
			trailing += p.x[i] * (p.y[i-1] - p.y[0])
		} else if i == 0 {
			trailing += p.x[i] * (p.lastY() - p.y[i+1])
		} else {
			trailing += p.x[i] * (p.y[i-1] - p.y[i+1])
		}
	}
	area := trailing / 2
